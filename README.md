...
- **Alias and skin** — replacements in the profile, TAB, nametag, and all text sent to clients; players still see their own skin. Message colors and formatting are preserved. With `names.hide_realname` (default on) the real name is not resolvable as an online player while incognito is active (`/msg`, `/tp`, selectors, `Bukkit.getPlayerExact`); the alias is, and offline lookups (bans, LuckPerms, whitelist) keep working with the real name.
