...
class ProfileIdsTest {
    ...
    fun `profile and entity identifiers agree and spectator targets are restored`() {
        ...
        assertEquals(identity.id, profiles.inbound(sent, identities))
    ...
    fun `removal uses the previously sent uuid across enabling and disabling`() {
        ...
        assertEquals(identity.id, profiles.profile(identity.id, identities, false))
        ...
        assertEquals(identity.maskedId, profiles.profile(identity.id, emptyList(), false))
