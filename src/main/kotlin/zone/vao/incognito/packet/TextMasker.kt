...
class TextMasker(private val patterns: List<Regex>) {
    ...
    fun mask(text: String, identities: List<Identity>, offset: CoordinateOffset): String {
        ...
        if (identities.isNotEmpty()) {
            val names = identities.associate { it.realName.lowercase() to it.alias }
            result = names(identities).replace(result) { names.getValue(it.value.lowercase()) }
        }
        if (offset != CoordinateOffset.ZERO) patterns.forEach { pattern -> result = pattern.replace(result) { shift(it, offset) } }
        return result
    }

    fun exposes(text: String, identities: List<Identity>): Boolean = identities.isNotEmpty() && names(identities).containsMatchIn(text)

    private fun names(identities: List<Identity>): Regex =
